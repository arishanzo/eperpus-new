...
<div class="flex min-h-screen w-full flex-wrap items-stretch bg-white max-md:pb-20 max-md:pt-32">
    <div class="grow md:flex md:w-1/2  md:flex-col md:items-center md:justify-center md:py-20">
        <div class="w-full  px-4 text-center text-xs lg:w-1/2">
            <h1 class="mb-20 text-2xl font-bold text-gray-800">Register E-Perpus</h1>
         
            <form class="flex flex-col gap-6" action="{{ route('register') }}" method="POST">
                @csrf

                <div class="relative mb-4">
                    <label class="flex cursor-pointer items-center gap-2 text-xs font-medium leading-none text-gray-700  mb-3">
                ...
                <div class="relative mb-4">
                    <label class="flex cursor-pointer items-center gap-2 text-xs font-medium leading-none text-gray-700  mb-3" >
                        <span class="font-semibold text-md">Nama User</span>
                        @error('name') <span style="color:red;">{{ $message }}</span> @enderror
                    </label>
                    <input id="username" type="text" name="name" class="block peer w-full px-4 py-3 border border-gray-300 bg-gray-100 text-gray-800 placeholder-gray-500  placeholder-gray-400 transition-colors focus:border-indigo-500 focus:outline-0 focus:ring focus:ring-indigo-200 " name="" value="" type="email" placeholder="Masukan Username Anda">
